...
import {allocidentity} from './base.js';
...
export function pinv(A) {
  ...
  let matrix = numeric.allocclone(A);
  let tmatrix = numeric.transpose(matrix);

  let matrix3 = numeric.dot(tmatrix, matrix);
  ...
  if (!ok) {
    let B = numeric.allocmat(A.length, A[0].length);
  ...
  S = numeric.allocdiag(S);
  let inv = numeric.dot(s.V, S);
  inv = numeric.dot(inv, numeric.transpose(s.U));

  inv = numeric.dot(inv, tmatrix);

...
export function dotMMbig(x, y) {
  var gc = _getCol, p = y.length, v = Array(p);
...
export const _foreach = (function _foreach(x, s, k, f) {
  if (k === s.length - 1) {
    f(x);
    return;
  }
...
export const mathfuns = ['abs', 'acos', 'asin', 'atan', 'ceil', 'cos',
                         'exp', 'floor', 'log', 'round', 'sin', 'sqrt', 'tan',
                         'isNaN', 'isFinite'];
export const mathfuns2 = ['atan2', 'pow', 'max', 'min'];
export const ops1 = {
  neg  : '-',
  not  : '!',
  bnot : '~',
  ...
  for (i in ops2) {
    if (ops2.hasOwnProperty(i)) {
      ...
      generated[i] = Function(
        ...
        '  y = arguments[i];\n' +
        '  if(typeof x === "object") {\n' +
        '      if(typeof y === "object") x = _biforeach2(x,y,dim(x),0,VV);\n' +
...
export function trunc(x, y) {
  if (typeof x === "object") {
    if (typeof y === "object") return truncVV(x, y);
    return truncVS(x, y);
  }
  if (typeof y === "object") return truncSV(x, y);
...
T.prototype.sub = Tbinop(
  'sub(x.x,y.x)',
  'sub(x.x,y.x),neg(y.y)',
  'sub(x.x,y.x),x.y',
  'sub(x.x,y.x),sub(x.y,y.y)');
T.prototype.mul = Tbinop(
  'mul(x.x,y.x)',
...
T.prototype.cos = Tunop(
  'return new T(generated.cos(x.x))',
...
T.prototype.inv = function inv() {
  ...
  var Rx = identity(n), Ry = rep([n, n], 0);
  var Ax = clone(A.x), Ay = clone(A.y);
...
export function toUpperHessenberg(me) {
  ...
  var m = s[0], i, j, k, x, v, A = clone(me), B, C, Ai, Ci, Q = identity(m), Qi;
  for (j = 0; j < m - 2; j++) {
    x = Array(m - j - 1);
...
export function QRFrancis(H, maxiter) {
  ...
  H = clone(H);
  //var H0 = clone(H);
  var s = dim(H), m = s[0], x, v, a, b, c, d, det, tr, Hloc, Q = identity(m), Qi, Hi, B, C, Ci, i, j, k,
      iter;
  ...
  for (iter = 0; iter < maxiter; iter++) {
    ...
    if (tr*tr >= 4*det) {
      ...
      Hloc = generated.add(generated.sub(generated.dot(Hloc, Hloc),
        generated.mul(Hloc, s1 + s2)),
        generated.diag(rep([3], s1*s2)));
    } else {
      Hloc = generated.add(generated.sub(generated.dot(Hloc, Hloc),
        generated.mul(Hloc, tr)),
        generated.diag(rep([3], det)));
    }
...
export const ccsLUP = ccsLUP0;

export function ccsDim(A) {
  return [generated.sup(A[1]) + 1, A[0].length - 1];
}
...
(function () {
  ...
  for (k in ops2) {
    ...
    if (isFinite(eval('0' + ops2[k] + '1'))) B = '[X[0],X[1],generated.' + k + '(X[2],Y)]';
    else B = 'NaN';
    if (isFinite(eval('1' + ops2[k] + '0')) && isFinite(eval('0' + ops2[k] + '1'))) C = 'generated.ccs' + k + 'MM(X,Y)';
    else C = 'NaN';
    generated['ccs' + k + 'MM'] = ccsbinop('zk = xk ' + ops2[k] + 'yk;');
    generated['ccs' + k] = Function('X', 'Y',
...
export function sidentity(n) {
  return sdiag(rep([n], 1));
}
...
export function sdot(A, B) {
  ...
  switch (k) {
    case 0:
      return A*B;
    case 1001:
      return generated.sdotVV(A, B);
    case 2001:
      return generated.sdotMV(A, B);
    case 1002:
      return generated.sdotVM(A, B);
    case 2002:
      return generated.sdotMM(A, B);
    default:
...
export function Spline(x, yl, yr, kl, kr) {
  this.x = x;
  this.yl = yl;
  this.yr = yr;
  this.kl = kl;
...
Spline.prototype.diff = function diff() {
  ...
  return new Spline(x, zl, zr, pl, pr);
}
Spline.prototype.roots = function roots() {
  function sqr(x) {
    return x*x;
  }
...
export function spline(x, y, k1, kn) {
  ...
  if (typeof k1 === "string") {
    ...
    for (i = k.length - 1; i !== -1; --i) {
      k[i] = cLUsolve(cLU(T), b[i]);
    }
  }
  if (typeof y[0] === "number") k = k[0];
  else k = transpose(k);
  return new Spline(x, y, y, k, k);
}
...
export function ifftpow2(x, y) {
  _ifftpow2(x, y);
  generated.diveq(x, x.length);
  generated.diveq(y, y.length);
}
...
T.prototype.fft = function fft() {
  ...
  X = X.mul(Y);
  convpow2(X.x, X.y, clone(Y.x), generated.neg(Y.y));
  X = X.mul(Y);
  X.x.length = n;
  X.y.length = n;
  return X;
...
export function gradient(f, x) {
  ...
  var max = Math.max;
  var i, x0 = clone(x), f1, f2, J = Array(n);
...
export function uncmin(f, x0, tol, gradient, maxit, callback, options) {
  ...
  tol = max(tol, epsilon);
  var step, g0, g1, H1 = options.Hinv || identity(n);
...
export function Dopri(x, y, f, ymid, iterations, msg, events) {
  this.x = x;
  this.y = y;
  this.f = f;
  this.ymid = ymid;
...
export function MPStoLP(MPS) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (j < states.length) {
      state = j;
      if (j === 1) { name = w[1]; }
      if (j === 6) return {name: name, c: c, A: transpose(A), b: b, rows: rows, vars: vars};
      continue;
    }
    switch (state) {
      case 0:
